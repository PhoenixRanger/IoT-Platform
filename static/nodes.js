...
let fleetNodes = [], groups = [], tags = [], capabilityRegistry = [], selectionMode = false, currentBulkAction = null;
let activeFilterFamily = null;
const selectedNodeIds = new Set();
const filters = {group: new Set(), tag: new Set(), capability: new Set(), status: new Set(), health: new Set()};
const filterLabels = {group: "Group", tag: "Tag", capability: "Capability", status: "Runtime Status", health: "Overall Health"};
...
function visibleNodes() {
 ...
 return fleetNodes.filter(node => (!query || node.node_id.toLocaleLowerCase().includes(query) || node.name.toLocaleLowerCase().includes(query))
  ...
  && matchesSet(filters.capability, node.expected_capabilities) && matchesSet(filters.status, [node.status]) && matchesSet(filters.health, [node.health]));
}
...
function filterOptions() { return {group: groups.map(item => [item.id, item.name]), tag: tags.map(item => [item.id, item.name]), capability: capabilityRegistry.map(item => [item.capability_key, item.display_name]), status: runtimeStatusOptions.map(item => [item, healthLabel(item)]), health: overallHealthOptions.map(item => [item, healthLabel(item)])}; }
...
function renderFilterFamilyValues(panel, family) {
 ...
 options.forEach(([value, label]) => {
  const wrapper = document.createElement("label"), input = document.createElement("input"); wrapper.dataset.filterLabel = label.toLocaleLowerCase(); input.type = "checkbox"; input.checked = filters[family].has(String(value)); input.onchange = () => { input.checked ? filters[family].add(String(value)) : filters[family].delete(String(value)); renderActiveFilters(); renderFleet(); }; wrapper.append(input, document.createTextNode(label)); optionList.append(wrapper);
 });
 if (["group", "tag", "capability"].includes(family)) {
